...
library(knitr)
kable(Significancia_MACS, col.names = colnames(Significancia_MACS), caption = 'Tabla 1')
```
...
```{r echo=FALSE}
kable(Dyads_MACS, col.names = colnames(Dyads_MACS), caption = 'Tabla 2')
```
...
El motivo de referencia siguiente corresponde a [FNR](http://goo.gl/qmBT0C).  
![PWM logo FNR](http://goo.gl/n3FClp)   
Se analizaron con RSAT-*Peak motifs* las secuencias arrojadas por el análisis de SWEMBL, tras haber procesados los datos con diferentes valores de gradiente (-R), también llamado valor relativo del background. Los valores R fueron: 0.5, 0.2, 0.1, 0.07, 0.06 y 0.05.
...
library(knitr)
kable(ME_maches, align = 'c', caption = 'Tabla 3',
      col.names = colnames(ME_maches))
```

En primera instancia podemos observar (**Tabla 3**) que con un parámetro de R inicial de 0.5 se encuentra el motivo de referencia dentro de los resultados y conforme decrece esta aparece un motivo devuelto por el oligo-análisis (calcula las ocurrencias de oligonucleótidos en un set de secuencias y detecta los sobrerepresentados) hasta llegar una R de 0.06 que no devuelve ninguna coincidencia con el motivo de referencia.   
...
Sin embargo se observa (**Figura 4**) que la significancia es muy baja desde un inicio lo que hace que sospechemos de los datos obtenidos con este experimento, ya que estamos utilizando precisamente al motivo de referencia para corroborar, desde la obtención de la muestra hasta el procesamiento bioinformático, que el protocolo de _chIP-seq_ fue realizado correctamente.   
...
Cabe manecionar también que el número de sitios encontrados que muestran los logos de Schneider son muchísimos: 147, 230, 166 y 371, cuando en RegulonDB muestra que solo son 84 en E. coli. Esta discrepancia nos indica de nuevo que algo anda mal con los datos que obtuvimos de la extracción con FNR. Se entiende que 84 es el número de sitios que esperas encontrar en el genoma de *E. coli*, por lo que con estos valores hace sentido que haya valores de significancia muy bajos.

Los análisis con MACS2 son muy parecidos a estos, pero nos pareció mejor no incluirlos en este apartado, ya que se hace un análisis más extenso de ellos en las demás secciones.
...
Después de comparar los resultados obtenidos con SWEMBL (y algunos con MACS2) podemos decir que sí encontramos el motivo de referencia dentro de nuestro conjunto de datos, por lo que, sólo basados en este criterio, podría validarse el buen seguimiento del protocolo ChIP-seq. Sin embargo, dado que tenemos valor de significacia y **correlación** poco concluyentes se refuerza la idea (como en el análisis de significacia) de que pudo haber salido algo mal en la fase de laboratorio o en el procesamiento de los datos. Se apuesta más por la **obtención de datos del *input***, aunque evidentemente debería revisarse cada aspecto. Para ver cómo salió el procesamiento de datos o qué *peak caller* deberíamos usar, tenemos que tomar en cuenta el análisis de consistencia, pero no debemos suponer que éste nos determina que parámetros usar en un posterior análisis, ya que esta prueba es específica para este cojunto de datos.  


### Consistencia

Dados los resultados obtenidos, podemos decir que ambos algoritmos presentan una buena aproximación al obtener motivos y que, en cada uno, se recuperan las mismas secuencias (o motivos muy similares) con significancias diferentes. En SWEMBL, el cambio en el valor relativo del background nos dice que entre más pequeño es el background, las secuencias se verán sobrerepresentadas. Esto nos daría una alza en la significancia, que puede observarse bien. En cuanto a MACS, a medida que el valor de corte se hace más restrictivo, la probabilidad de encontrar un motivo baja y por tanto su significancia. Estos resultados son consistentes con lo que uno esperaría observar en los análisis.
