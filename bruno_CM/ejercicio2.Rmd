...
```{r echo = FALSE}
predicted_tss_s0.9 <- read.table(file = "./bruno_CM/predicted_TSS_0.97_-18.37.txt",
                            sep = "\t",
                            col.names = c("id","pos", "D", "E1", "E2", "result"))
predicted_tss_s0.7 <- read.table(file = "./bruno_CM/predicted_TSS_2.35_-18.07.txt",
                            sep = "\t",
                            col.names = c("id","pos", "D", "E1", "E2", "result"))
head(predicted_tss_s0.7)
```

### Graficamos $D(n)$, E1 y E2 para una posición $n$ al azar  
...
plot(x = predicted_tss_s0.7[predicted_tss_s0.7$id==id, 'pos'],
     ...
     xlim = c(-400, 50), xlabel = "Posición",
     ylim = c(-20, 5), type = "l", horiz = 0.2,
     ylabel = "Energía libre de Gibbs",
     main = "Para un n alaeatorio con sensibilida de 0.7")
